...
mysql_select_db('northernairport');
...
$sql = "SELECT r.reservationid, flighttime, (select name FROM airlines WHERE airlineid=returnairlineid) AS AirlineName, " .
	"flightnumber, '' as FlightCity, (select terminal FROM airlines WHERE airlineid=returnairlineid) AS TerminalName, " .
    "r.reservationid AS confirmationnumber,  returnnumchildren + returnnumstudents + returnnumadults + returnnumseniors as NumPax, " .
